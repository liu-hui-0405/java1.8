...
public class ListToAnotherList {
    public static void main(String[] args) {
        ...
        List<Student> stuList = personList.stream().filter(p -> p.getPersonType().equals("student"))
                .map(p -> new Student(p.getId(), p.getName()))
                .collect(Collectors.toList());

        stuList.forEach(s -> System.out.println("Id:"+ s.getId()+ ", Name: "+ s.getName()));
    }
}
class Person {
    private int id;
